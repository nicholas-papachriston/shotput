...
describe("directory", () => {
	...
	beforeEach(async () => {
		tempDir = `${process.cwd()}/test-temp-directory-${Date.now()}`;
	...
	describe("ensureDirectoryExists", () => {
		it("should create a directory if it does not exist", async () => {
			...
			// Directory should exist now (we can't directly check if directory exists with Bun.file,
			// but we can try to create a file in it)
	...
	describe("handleDirectory", () => {
		...
		it("should handle non-existent directory", async () => {
			...
			expect(result.operationResults).toContain("[Error reading directory");
			expect(result.combinedRemainingCount).toBe(10000);
		});
