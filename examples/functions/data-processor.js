...
export default async function dataProcessor(
	...
	const errorRate = (
		(sampleData.metrics.errors / sampleData.metrics.totalRequests) *
		100
	).toFixed(2);
	const successfulRequests = Math.round(
		(sampleData.metrics.totalRequests * sampleData.metrics.successRate) / 100,
	);
...
${departments
	.map(
		(dept) => `**${dept}**: ${byDepartment[dept].length} member(s)
