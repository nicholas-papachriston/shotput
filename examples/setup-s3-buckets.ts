  ...
  // Standard buckets for basic examples
  {
    ...
    files: [
      ...
      },
      {
        ...
        content: "[2024-01-15 11:00:01] INFO Processing request\n[2024-01-15 11:00:02] INFO Request completed\n"
      },
      {
        key: "logs/2024/app.log",
        content: "[2024-01-15] Application log entry\n[2024-01-16] Another log entry\n"
      },
      {
    ...
    files: [
      {
        ...
        content: "[2023-12-01] Archived log entry\n[2023-12-02] Another archived entry\n"
      }
    ]
  },
...
// Upload a file to S3
async function uploadFile(bucketName: string, key: string, content: string): Promise<boolean> {
  log.info(`  Uploading: s3://${bucketName}/${key}`);

  if (isDryRun) {
    log.info(`  [DRY RUN] Would upload ${content.length} bytes to s3://${bucketName}/${key}`);
    return true;
  }
...
async function main() {
  ...
  for (const bucket of targetBuckets) {
    ...
    log.info(`   ✓ Complete: ${uploaded}/${bucket.files.length} files uploaded`);
  }
  ...
  if (!isR2 && directoryBuckets.length > 0) {
    ...
    log.info("   These require special AWS configuration and must be created manually.");
